...
public interface ImageRepository extends JpaRepository<Image,Integer> {
	...
	@Query(value="select i.* from image i inner join (select imageId, count(imageId) as commentCount from comment group by imageId) c on i.id = c.imageId order by commentCount DESC",nativeQuery=true)
	List<Image> mPopular();
